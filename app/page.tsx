  ...
  return <main className="management">
    ...
    {view === "reservations" && <section className="panel management-panel">
      ...
      <div className="table-wrap"><table className="large-table">
        ...
        <tbody>{sortedReservations.map(r => <tr key={r.id} onClick={() => onSelect(r)}><td><strong>{r.id}</strong><small>{r.received}</small></td><td><span className={"badge " + statusClass[r.status]}><i/>{reservationDisplayLabel(r)}</span></td><td><strong>{r.customer}</strong><small>{r.phone}</small></td><td><strong>{reservationDateTimeLabel(r)}</strong><strong>{r.people}名</strong></td><td>{reservationMenuLabel(r)}</td><td>{assignmentLabel(r) ? <strong>{assignmentLabel(r)}</strong> : <span className="unassigned">未割当</span>}</td><td>{r.confirmationContactedAt ? <><strong>連絡済</strong><small>{new Date(r.confirmationContactedAt).toLocaleDateString("ja-JP")}</small></> : <span className="unassigned">未連絡</span>}</td></tr>)}</tbody>
    ...
    <div className="form-body narrow customer-reservation-dashboard">
      ...
      {isLoggedIn && (isLoading ? <div className="empty-table compact">予約情報を読み込んでいます。</div> : reservationError ? <div className="auth-error">{reservationError}</div> : reservations.length ? (
        <div className="customer-reservation-list">{reservations.map(reservation => <article key={reservation.id} className="customer-reservation-item">
          <div><span className={"badge " + statusClass[reservation.status]}><i/>{reservationDisplayLabel(reservation)}</span><strong>{reservationDateTimeLabel(reservation)}</strong><small>{reservation.people}名・{reservationMenuLabel(reservation)}</small></div>
