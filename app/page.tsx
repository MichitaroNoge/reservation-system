...
import type { BookingForm, Customer, CustomerForm, Menu, MenuForm, Reservation, ReservationFilter, ReservationSortKey, ReservationSubmitOptions, SortDirection, Status, Store, StoreAssignment, StoreForm, View } from "./reservations/types";
  ...
  return <main className="management">
    ...
    {view === "reservations" && <section className="panel management-panel">
      ...
      <div className="table-wrap"><table className="large-table">
        ...
        <tbody>{sortedReservations.map(r => <tr key={r.id} onClick={() => onSelect(r)}><td><strong>{r.id}</strong><small>{r.received}</small></td><td><span className={"badge " + statusClass[r.status]}><i/>{reservationDisplayLabel(r)}</span></td><td><strong>{r.customer}</strong><small>{r.phone}</small></td><td><strong>{reservationDateTimeLabel(r)}</strong><small>{r.people}名</small></td><td>{reservationMenuLabel(r)}</td><td>{assignmentLabel(r) ? <strong>{assignmentLabel(r)}</strong> : <span className="unassigned">未割当</span>}</td><td>{r.confirmationContactedAt ? <><strong>連絡済</strong><small>{new Date(r.confirmationContactedAt).toLocaleDateString("ja-JP")}</small></> : <span className="unassigned">未連絡</span>}</td><td><Icon name="arrow"/></td></tr>)}</tbody>
