...
function CustomerPortalHome({ customerEmail, isLoggedIn, authError, accountEmail, accountPassword, accountSubmitting, customerAuthLoading, onAccountEmailChange, onAccountPasswordChange, onLogin, onLogout, onOpenAccount, onOpenReservation, onOpenConfirmedChange, onOpenChange, onOpenCancellation }: { customerEmail: string; isLoggedIn: boolean; authError: string; accountEmail: string; accountPassword: string; accountSubmitting: boolean; customerAuthLoading: boolean; onAccountEmailChange: (value: string) => void; onAccountPasswordChange: (value: string) => void; onLogin: () => void; onLogout: () => void; onOpenAccount: () => void; onOpenReservation: () => void; onOpenConfirmedChange: () => void; onOpenChange: () => void; onOpenCancellation: () => void }) {
  return <div className="form-body narrow portal-entry">
    ...
    <section className="customer-account-panel portal-account-panel">
      {isLoggedIn ? (
        ...
        <>
          <p>ログインすると、各種手続きをログイン状態で進められます。</p>
    ...
    <div className="portal-entry-grid"><button type="button" onClick={onOpenAccount}><strong>予約確認</strong><small>ログインして自分の予約を確認します</small></button><button type="button" onClick={onOpenReservation}><strong>予約申請</strong><small>仮予約または本予約を申し込みます</small></button><button type="button" onClick={onOpenConfirmedChange}><strong>本予約への変更申請</strong><small>確定済みの仮予約を本予約へ変更申請します</small></button><button type="button" onClick={onOpenChange}><strong>予約内容変更申請</strong><small>受付済みの予約について日時・人数・メニューの変更を申請します</small></button><button type="button" onClick={onOpenCancellation}><strong>キャンセル申請</strong><small>受付済みの予約のキャンセルを申請します</small></button></div>
  </div>;
}

function CustomerReservationDashboard({ customerEmail, isLoggedIn, authError, accountEmail, accountPassword, accountSubmitting, customerAuthLoading, reservations, changeRequests, isLoading, reservationError, canRequestConfirmedChange, canRequestChange, canRequestCancellation, onBack, onAccountEmailChange, onAccountPasswordChange, onLogin, onLogout, onConfirmedChange, onChange, onCancellation }: { customerEmail: string; isLoggedIn: boolean; authError: string; accountEmail: string; accountPassword: string; accountSubmitting: boolean; customerAuthLoading: boolean; reservations: Reservation[]; changeRequests: ReservationChangeRequest[]; isLoading: boolean; reservationError: string; canRequestConfirmedChange: (reservation: Reservation) => boolean; canRequestChange: (reservation: Reservation) => boolean; canRequestCancellation: (reservation: Reservation) => boolean; onBack: () => void; onAccountEmailChange: (value: string) => void; onAccountPasswordChange: (value: string) => void; onLogin: () => void; onLogout: () => void; onConfirmedChange: (reservation: Reservation) => void; onChange: (reservation: Reservation) => void; onCancellation: (reservation: Reservation) => void }) {
  return <>
...
function CustomerPortal({ initialMode, form, setForm, step, setStep, onAdmin, notify, toast, onSubmitReservation, onSubmitCancellation, onSubmitConfirmedReservationChange, onSubmitChangeRequest, menuCatalog }: { initialMode: CustomerPortalMode; form: BookingForm; setForm: Dispatch<SetStateAction<BookingForm>>; step:number; setStep:(n:number)=>void; onAdmin:()=>void; notify:(s:string)=>void; toast:string; onSubmitReservation:(form: BookingForm, options?: ReservationSubmitOptions)=>Promise<Reservation>; onSubmitCancellation:(input: { reservationId: string; email?: string; phone?: string }, options?: { authToken?: string })=>Promise<Reservation>; onSubmitConfirmedReservationChange:(input: { reservationId: string; email?: string; phone?: string }, options?: { authToken?: string })=>Promise<Reservation>; onSubmitChangeRequest:(input: { reservationId: string; email?: string; phone?: string; requestedDate: string; requestedStartTime: string; requestedPeople: number; requestedMenuItems: string[]; reason?: string }, options?: { authToken?: string })=>Promise<ReservationChangeRequest>; menuCatalog: Menu[] }) {
  ...
  return <main className="customer-page restaurant-reservation">
    ...
    <section className="booking-card">
      {portalMode === "home" && <CustomerPortalHome customerEmail={customerUser?.email ?? ""} isLoggedIn={Boolean(customerUser)} authError={customerAuthError} accountEmail={accountEmail} accountPassword={accountPassword} accountSubmitting={accountSubmitting} customerAuthLoading={customerAuthLoading} onAccountEmailChange={setAccountEmail} onAccountPasswordChange={setAccountPassword} onLogin={loginForAccountReservations} onLogout={() => { setAccountReservations([]); setAccountChangeRequests([]); signOutCustomer(); }} onOpenAccount={() => { setPortalMode("account"); setAccountReservationError(""); }} onOpenReservation={() => { setPortalMode("reservation"); setStep(1); }} onOpenConfirmedChange={() => { setPortalMode("confirmedChange"); setConfirmedChangeSubmitted(false); }} onOpenChange={() => { setPortalMode("change"); setChangeRequestSubmitted(false); }} onOpenCancellation={() => { setPortalMode("cancellation"); setCancellationSubmitted(false); }} />}
      ...
      {portalMode === "reservation" && <><button className="portal-back-button" type="button" onClick={backToPortalHome}>手続き選択へ戻る</button><div className="stepper">{["利用者区分","予約種別","日時・人数","お客様情報","受付完了"].map((label, index)=><div key={label} className={step >= index + 1 ? "active" : ""}><span>{step > index + 1 ? <Icon name="check"/> : index + 1}</span><small>{label}</small>{index < 4 && <i/>}</div>)}</div>
      ...
      {step === 3 && <div className="form-body narrow"><p className="form-kicker">STEP 3</p><h2>日時と人数</h2><div className="form-fields reservation-date-fields"><label>利用日<input type="date" value={form.date} onChange={event => setForm({ ...form, date: event.target.value })}/></label><label>開始時間<input type="time" value={form.startTime} onChange={event => setForm({ ...form, startTime: event.target.value })}/></label><label>人数<input type="number" min="1" max="999" value={form.people} onChange={event => setForm({ ...form, people: Number(event.target.value) })}/></label></div><MenuPicker menuCatalog={menuCatalog} selected={form.menuItems} onChange={menuItems => setForm({ ...form, menuItems })}/><div className="reservation-summary"><strong>終了予定 {bookingFormEndTime(form, menuCatalog)}</strong><span>{bookingFormDateTimeLabel({ ...form, endTime: bookingFormEndTime(form, menuCatalog) })}・{form.people}名</span></div><div className="form-nav"><button onClick={() => setStep(2)}>戻る</button><button className="next" disabled={!form.date || !form.startTime || !form.people} onClick={() => setStep(4)}>お客様情報へ <Icon name="arrow"/></button></div></div>}
