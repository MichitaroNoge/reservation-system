...
import { assignmentLabel, bookingFormDateTimeLabel, bookingFormEndTime, buildCustomers, dateHeadingLabel, daysUntilVisit, fullDateHeadingLabel, isConfirmationContactDue, isTemporaryReservationExpired, menuSelectionLabel, monthIso, policyAgreementLabel, reservationBookingTypeLabel, reservationBookingTypeShortLabel, reservationCustomerSubLabel, reservationDateTimeLabel, reservationDisplayLabel, reservationMenuLabel, reservationStartTime, selectedMenuTotal, statusLabel, todayIso } from "./reservations/formatters";
...
function ReservationBookingTypeCell({ reservation }: { reservation: Reservation }) {
  return <><strong>{reservationBookingTypeShortLabel(reservation)}</strong>{reservation.bookingType === "travel_agency_group" && reservation.groupName ? <small>{reservation.groupName}</small> : null}</>;
}
...
function NewReservationDrawer({ form, setForm, onClose, onSubmit, menuCatalog }: { form: BookingForm; setForm: Dispatch<SetStateAction<BookingForm>>; onClose: () => void; onSubmit: () => void; menuCatalog: Menu[] }) {
  ...
  return <><div className="drawer-shade" onClick={onClose}/><aside className="drawer new-reservation-drawer"><header><div><span className="badge blue"><i/>新規登録</span><h2>新規予約</h2></div><button onClick={onClose}><Icon name="close"/></button></header><div className="drawer-body">
    ...
    <section className="drawer-primary-action"><p className="section-label">任意メニュー</p><MenuPicker menuCatalog={menuCatalog} selected={form.menuItems} onChange={menuItems => setForm({ ...form, menuItems, endTime: bookingFormEndTime({ ...form, menuItems, endTime: undefined }, menuCatalog) })}/><PaymentAndRemarksFields form={form} setForm={setForm} /><div className="reservation-summary"><strong>{menuSelectionLabel(form.menuItems)}</strong><span>{form.date && form.startTime && form.people ? `${bookingFormDateTimeLabel({ ...form, endTime: form.endTime ?? bookingFormEndTime(form, menuCatalog) })}・${form.people}名` : "日時・人数を入力してください"}</span><small>{"¥"}{total.toLocaleString()}</small></div><button className="full-action" disabled={!canSubmit} onClick={onSubmit}><Icon name="check"/>予約を登録する</button></section>
  </div></aside></>;
}
