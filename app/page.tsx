...
import { assignmentLabel, bookingFormDateTimeLabel, buildCustomers, dateHeadingLabel, daysUntilVisit, fullDateHeadingLabel, isConfirmationContactDue, isTemporaryReservationExpired, menuSelectionLabel, monthIso, policyAgreementLabel, reservationCustomerSubLabel, reservationDateTimeLabel, reservationDisplayLabel, reservationMenuLabel, reservationStartTime, selectedMenuTotal, statusLabel, todayIso } from "./reservations/formatters";
...
export default function Home() {
  ...
  const [form, setForm] = useState<BookingForm>({ menuItems: [], date: "2026-07-12", startTime: DEFAULT_START_TIME, people: 2, name: "", email: "", phone: "", address: "", accountType: "individual", bookingType: "individual", status: STATUS.confirmedRequested });
  ...
  const [adminForm, setAdminForm] = useState<BookingForm>({ menuItems: [], date: "2026-07-12", startTime: DEFAULT_START_TIME, people: 2, name: "", email: "", phone: "", address: "", accountType: "individual", bookingType: "individual", status: STATUS.confirmed });
  ...
  const submitAdminReservation = async () => {
    try {
      ...
      setAdminForm({ menuItems: [], date: "2026-07-12", startTime: DEFAULT_START_TIME, people: 2, name: "", email: "", phone: "", address: "", accountType: "individual", bookingType: "individual", status: STATUS.confirmed });
  ...
  return <main className="management">
    ...
    {view === "reservations" && <section className="panel management-panel">
      ...
      <div className="table-wrap"><table className="large-table">
        ...
        <tbody>{sortedReservations.map(r => <tr key={r.id} onClick={() => onSelect(r)}><td><strong>{r.id}</strong><small>{r.received}</small></td><td><span className={"badge " + statusClass[r.status]}><i/>{reservationDisplayLabel(r)}</span></td><td><strong>{r.customer}</strong><small>{reservationCustomerSubLabel(r)}</small></td><td><strong>{reservationDateTimeLabel(r)}</strong><strong>{r.people}名</strong></td><td>{reservationMenuLabel(r)}</td><td>{assignmentLabel(r) ? <strong>{assignmentLabel(r)}</strong> : <span className="unassigned">未割当</span>}</td><td>{r.confirmationContactedAt ? <><strong>連絡済</strong><small>{new Date(r.confirmationContactedAt).toLocaleDateString("ja-JP")}</small></> : <span className="unassigned">未連絡</span>}</td></tr>)}</tbody>
...
function CustomerContactRequestFields<T extends CustomerContactRequestForm>({ form, onFormChange }: { form: T; onFormChange: Dispatch<SetStateAction<T>> }) {
  return <div className="form-fields single"><label>予約ID<input value={form.reservationId} placeholder="例: RSV-1047" onChange={event => onFormChange({ ...form, reservationId: event.target.value })}/></label><label>メールアドレス<input type="email" value={form.email} onChange={event => onFormChange({ ...form, email: event.target.value })}/></label><label>電話番号<input value={form.phone} onChange={event => onFormChange({ ...form, phone: event.target.value })}/></label></div>;
}
...
function ReservationDrawer({ reservation: r, onClose, updateStatus, updateConfirmationContact, assignStores, updateReservation, menuCatalog, stores }: { reservation: Reservation; onClose: () => void; updateStatus: (id: string, status: Status, options?: { manualReason?: string }) => void; updateConfirmationContact: (id: string, contactedAt: string | null, options?: { sendEmail?: boolean }) => Promise<void>; assignStores: (id: string, assignments: StoreAssignment[]) => void; updateReservation: (id: string, form: BookingForm) => Promise<Reservation>; menuCatalog: Menu[]; stores: Store[] }) {
  const initialEditForm = (): BookingForm => ({ menuItems: r.menuItems ?? (r.menu ? [r.menu] : []), date: r.date, startTime: r.startTime ?? DEFAULT_START_TIME, people: r.people, name: r.customer, email: r.email ?? "", phone: r.phone, address: r.address ?? "", bookingType: r.bookingType ?? "individual", bookingContactName: r.bookingContactName ?? "", dayContactName: r.dayContactName ?? "", dayContactPhone: r.dayContactPhone ?? "", groupName: r.groupName ?? "", groupNameKana: r.groupNameKana ?? "", groupType: r.groupType ?? "", groupTypeOther: r.groupTypeOther ?? "", status: r.status });
...
function CustomerPortal({ initialMode, form, setForm, step, setStep, onAdmin, notify, toast, onSubmitReservation, onSubmitCancellation, onSubmitConfirmedReservationChange, onSubmitChangeRequest, menuCatalog }: { initialMode: CustomerPortalMode; form: BookingForm; setForm: Dispatch<SetStateAction<BookingForm>>; step:number; setStep:(n:number)=>void; onAdmin:()=>void; notify:(s:string)=>void; toast:string; onSubmitReservation:(form: BookingForm, options?: ReservationSubmitOptions)=>Promise<Reservation>; onSubmitCancellation:(input: { reservationId: string; email?: string; phone?: string }, options?: { authToken?: string })=>Promise<Reservation>; onSubmitConfirmedReservationChange:(input: { reservationId: string; email?: string; phone?: string }, options?: { authToken?: string })=>Promise<Reservation>; onSubmitChangeRequest:(input: { reservationId: string; email?: string; phone?: string; requestedDate: string; requestedStartTime: string; requestedPeople: number; requestedMenuItems: string[]; reason?: string }, options?: { authToken?: string })=>Promise<ReservationChangeRequest>; menuCatalog: Menu[] }) {
  ...
  const total = selectedMenuTotal(form.menuItems, menuCatalog);
  const groupReady = !isGroupBooking(form) || Boolean(form.companyBranchName && form.bookingContactName && form.dayContactName && form.dayContactPhone && form.groupName && form.groupType && bookingMode !== "guest");
  const canSubmit = Boolean(form.name && form.email && form.phone && form.date && form.startTime && form.people && groupReady);
  ...
  const submit = async () => {
    ...
    try {
      ...
      const submitForm: BookingForm = isGroupBooking(form)
        ...
        : { ...form, accountType: "individual", bookingType: "individual" };
    ...
    <section className="booking-card">
      ...
      {step === 3 && <div className="form-body narrow"><p className="form-kicker">STEP 3</p><h2>お客様情報</h2>
        <section className="customer-account-panel">
          ...
          {bookingMode === "guest" ? <p className="customer-mode-note">{isGroupBooking(form) ? "旅行代理店の団体予約は、ログインまたはアカウント登録が必要です。" : "アカウント登録なしでも予約申請できます。"}</p> : customerUser ? (
