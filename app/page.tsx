...
export default function Home() {
  ...
  const [form, setForm] = useState<BookingForm>({ menuItems: [], date: "2026-07-12", startTime: DEFAULT_START_TIME, people: 2, name: "", email: "", phone: "", address: "", status: STATUS.confirmedRequested });
  ...
  const [adminForm, setAdminForm] = useState<BookingForm>({ menuItems: [], date: "2026-07-12", startTime: DEFAULT_START_TIME, people: 2, name: "", email: "", phone: "", address: "", status: STATUS.confirmed });
  ...
  const submitAdminReservation = async () => {
    try {
      ...
      setAdminForm({ menuItems: [], date: "2026-07-12", startTime: DEFAULT_START_TIME, people: 2, name: "", email: "", phone: "", address: "", status: STATUS.confirmed });
...
function ReservationDrawer({ reservation: r, onClose, updateStatus, updateConfirmationContact, assignStores, updateReservation, menuCatalog, stores }: { reservation: Reservation; onClose: () => void; updateStatus: (id: string, status: Status, options?: { manualReason?: string }) => void; updateConfirmationContact: (id: string, contactedAt: string | null, options?: { sendEmail?: boolean }) => Promise<void>; assignStores: (id: string, assignments: StoreAssignment[]) => void; updateReservation: (id: string, form: BookingForm) => Promise<Reservation>; menuCatalog: Menu[]; stores: Store[] }) {
  const initialEditForm = (): BookingForm => ({ menuItems: r.menuItems ?? (r.menu ? [r.menu] : []), date: r.date, startTime: r.startTime ?? DEFAULT_START_TIME, people: r.people, name: r.customer, email: r.email ?? "", phone: r.phone, address: r.address ?? "", status: r.status });
