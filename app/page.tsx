...
function CustomerPortal({ form, setForm, step, setStep, onAdmin, notify, toast, onSubmitReservation, menuCatalog }: { form: BookingForm; setForm: React.Dispatch<React.SetStateAction<BookingForm>>; step:number; setStep:(n:number)=>void; onAdmin:()=>void; notify:(s:string)=>void; toast:string; onSubmitReservation:(form: BookingForm)=>Promise<Reservation>; menuCatalog: Menu[] }) {
  ...
  return <main className="customer-page"><header><div className="public-logo"><span>R</span><strong>Reserve</strong></div><nav><a href="#guide">ご利用ガイド</a><a href="#contact">お問い合わせ</a><button onClick={onAdmin}>管理画面</button></nav></header><section className="customer-hero"><div><p>ONLINE RESERVATION</p><h1>あなたにぴったりの時間を、<br/><em>かんたん予約。</em></h1><span>日時と人数を選んで、オンラインで予約を申請できます。メニューは任意で事前選択できます。</span></div><div className="hero-orb"><Icon name="calendar"/></div></section><section className="booking-card"><div className="stepper">{["日時・人数","お客様情報","任意メニュー","受付完了"].map((s,i)=><div key={s} className={step >= i+1 ? "active" : ""}><span>{step > i+1 ? "✓" : i+1}</span><small>{s}</small>{i<3&&<i/>}</div>)}</div>
    ...
    {step === 3 && <div className="form-body"><p className="form-kicker">STEP 3</p><h2>予約種別とメニューを選択</h2><p>本予約として申し込むか、仮予約として相談するかを選べます。メニューは未選択でも予約できます。</p><div className="reservation-type-grid"><button className={form.status === "本予約申請中" ? "selected" : ""} onClick={() => setForm({ ...form, status: "本予約申請中" })}><span>{form.status === "本予約申請中" && <Icon name="check"/>}</span><strong>本予約を申し込む</strong><small>来店前提で予約を申請します</small></button><button className={form.status === "仮予約申請中" ? "selected" : ""} onClick={() => setForm({ ...form, status: "仮予約申請中" })}><span>{form.status === "仮予約申請中" && <Icon name="check"/>}</span><strong>仮予約として相談する</strong><small>内容確認後に本予約へ進めます</small></button></div><MenuPicker menuCatalog={menuCatalog} selected={form.menuItems} onChange={menuItems=>setForm({...form,menuItems})}/><div className="confirm-box"><span>{form.date.replaceAll("-","/")}</span><strong>{menuSelectionLabel(form.menuItems)}・{form.people}名</strong><small>{form.status ?? "本予約申請中"}・¥{total.toLocaleString()}（税込）</small></div><div className="form-nav"><button onClick={()=>setStep(2)}>戻る</button><button className="next" onClick={submit}>予約を申請する <Icon name="arrow"/></button></div></div>}
    {step === 4 && <div className="form-body complete"><span><Icon name="check"/></span><p className="form-kicker">REQUEST RECEIVED</p><h2>予約申請を受け付けました</h2><p>管理部門で内容を確認後、確定メールをお送りします。</p><button className="next" onClick={()=>{setStep(1);setForm({...form,name:"",email:"",phone:"",menuItems:[],status:"本予約申請中"})}}>トップに戻る</button></div>}
  </section><section className="guide" id="guide"><div><span>01</span><h3>予約を申請</h3><p>日時と人数、お客様情報を入力します。メニューは任意です。</p></div><div><span>02</span><h3>運営が内容を確認</h3><p>空き状況を確認し、受付可否をメールでご連絡します。</p></div><div><span>03</span><h3>予約確定</h3><p>本予約の承認後、担当店舗をご案内します。</p></div></section>{toast&&<div className="toast"><Icon name="check"/>{toast}</div>}</main>;
}
