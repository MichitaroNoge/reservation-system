...
function CustomerPortalHome({ customerEmail, isLoggedIn, authError, accountEmail, accountPassword, accountSubmitting, customerAuthLoading, onAccountEmailChange, onAccountPasswordChange, onLogin, onLogout, onOpenAccount, onOpenReservation }: { customerEmail: string; isLoggedIn: boolean; authError: string; accountEmail: string; accountPassword: string; accountSubmitting: boolean; customerAuthLoading: boolean; onAccountEmailChange: (value: string) => void; onAccountPasswordChange: (value: string) => void; onLogin: () => void; onLogout: () => void; onOpenAccount: () => void; onOpenReservation: () => void }) {
  return <div className="form-body narrow portal-entry">
    ...
    <div className="portal-entry-grid"><button type="button" onClick={onOpenReservation}><strong>予約申請</strong><small>仮予約または本予約を申し込みます</small></button><button type="button" onClick={onOpenAccount}><strong>予約確認・変更・キャンセル</strong><small>予約の確認、変更申請、キャンセル申請を行います</small></button></div>
  </div>;
}
